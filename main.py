...
def ket_0(U, qubits):
    ...
    U.append(
        temp_U.to_instruction(label="\\ket{0}\\textsubscript{\\textit{L}}"), qubits
    )
...
def ket_1(U, qubits):
    ...
    U.append(
        temp_U.to_instruction(label="\\ket{1}\\textsubscript{\\textit{L}}"), qubits
    )
...
def iterate_single_qubit_errors():
    ...
    for i in range(15):
        ...
        assert (
            len(counts) == 1
        )  # assert that there is only one unique syndrome measurement for each single qubit error
        syndromes.append(list(counts.keys())[0])
    df = pd.DataFrame(
        syndromes,
        columns=df_columns,
        index=pd.Index(df_index, name="Single qubit error"),
    )
    return df
